...
const getters = {
  ...
  favouritesCharacters: (state: State) : Array<Character> => state.favouriteCharacters,
}

const actions = {
  setCharacters: async ({commit, state}: {commit: Commit , state: State}) : Promise<void> => {
  ...
  setSearchValue: ({commit}: {commit: Commit}, searchValue: string) : void => {
    commit('setSearchValue', searchValue);
  },
  addToFavourites: ({commit}: {commit: Commit}, character: Character) : void => {
  ...
  initFavourites: ({commit}: {commit: Commit}, characters: Characters) : void => {
    commit('initFavourites', characters);
  }
}

const mutations = {
  setCharacters: (state: State, characters: Characters) : void => {state.characters = characters},
  setPage: (state: State, page: number) : void => {state.page = page},
  setSearchValue: (state: State, searchValue: string) : void => {state.searchValue = searchValue},
  ...
  removeFromFavourites: (state: State, id: number) : void => {
    state.favouriteCharacters = state.favouriteCharacters.filter((char: Character) => +char.id !== id);
  },
  initFavourites: (state: State, characters: any) : void => {
    Object.keys(characters).forEach((key) => {
      if(isNaN(+key)) return;
      state.favouriteCharacters = [...state.favouriteCharacters ,JSON.parse(characters.getItem(key))];
   });
  }
}
